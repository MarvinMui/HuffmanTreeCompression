...
Tnode* make_huffman_tree(NodeL* head) {
	...
	while(head -> next != NULL) {
		...
		long new_size = ((Tnode*)(head -> a_value)) -> frequency + 
			((Tnode*)(head -> next -> a_value)) -> frequency;
		Tnode* left = ((Tnode*)(head -> a_value));
		Tnode* right = ((Tnode*)(head -> next -> a_value));
